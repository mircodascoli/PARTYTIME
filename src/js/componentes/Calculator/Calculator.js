...
export class Calculator extends LitElement {
 ...
 static styles = [ResetCSS, CalculatorCSS];
     ...
     return html`
      <div class="container-calculador">
        <div id="inputscalculador">
          ...
          <input id="range" type="range" min="1" max="10" step="1" value="5"
                 @input=${this.handleRange}/>
          <span id="people-units"> for: ${this.quantity}</span>
  ...
  handleRange(e) {
    console.log("qualcuno ha mosso il range")
    const target = e.target;
    this.quantity = parseInt(target.value);
    this.requestUpdate();
    console.log(this.quantity, "la quantity nel range");
  }
}
